...
export const getColumnsDimensions = (container: HTMLElement | null) => {
...
export const getDataAsRows = (data: Card[] | null, numberOfColumns: number) => {
...
export const getPositionRelativeToDocument = (element: Element | null) => {
...
export const getColumnHeightAtRow = (row: number, column: number, allCardElements: (Element | null)[][]) => {
...
export const sortElementsByHeight = (elements: (Element | null)[] | null) => {
  if (!elements) return [];
...
export const getTallestColumn = (container: HTMLElement | null, rows: Card[][]) => {
...
export const moveDataElement = (data: Card[], sourceId?: string, targetId?: string) => {
  if (!sourceId || !targetId) return data;
  let sourceIndex = data.findIndex(
    (card: Card) => card?.id === sourceId
  );
  let targetIndex = data.findIndex(
    (card: Card) => card?.id === targetId
  );
  ...
  data?.forEach((card: Card, index: number) => {
    ...
    if (index === targetIndex) {
      if (sourceIndex < targetIndex) {
        console.log(sourceIndex, targetIndex, data[sourceIndex], card);
